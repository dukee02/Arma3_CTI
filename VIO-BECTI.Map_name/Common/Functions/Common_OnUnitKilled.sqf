...
if ((_killer == _killed || isNull _killer) && _isvehicle_killed) then {
	...
	if !(isNil "_last_hit") then {
		if (alive _last_hit) then {
			...
			if (time - (_killed getVariable "cti_lasthit_time") < 60) then {_killed_proxy = true; _killer = _last_hit};
			//if (time - (_killed getVariable "cti_lasthit_time") < 25) then {_killed_proxy = true; _killer = _last_hit};
		};
	};
	//if (CTI_Log_Level >= CTI_Log_Debug) then { 
		//["VIOC_DEBUG", "FILE: common\functions\common_onunitkilled.sqf", format["no direkt kill, search for killer <_last_hit = %1 | cti_lasthit_time = %2 | _killer = %3>", _killed getVariable "cti_lasthit", (time - (_killed getVariable "cti_lasthit_time")), _killer]] call CTI_CO_FNC_Log;
		//["VIOC_DEBUG", "FILE: common\functions\common_onunitkilled.sqf", format["no direkt kill, search for killer <killed = %1 >", _killed ]] call CTI_CO_FNC_Log;
	//};
};
