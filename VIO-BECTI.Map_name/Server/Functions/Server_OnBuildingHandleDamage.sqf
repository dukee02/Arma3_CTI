...
if (_reduce_damages > 0) then {_currentdmg = getDamage _damaged; _damage = _currentdmg + ((_damage - _currentdmg) / _reduce_damages)};
...
if (time - (_logic getVariable "cti_structures_lasthit") > 30 && _damage >= 0.02 && alive _damaged) then {
	...
	if (CTI_Log_Level >= CTI_Log_Debug || CTI_DEBUG) then {
		["VIOC-DEBUG", "File: Server\Functions\Server_OnBuildingHandleDamage.sqf", format["Building %1 gets damage: <%2><%3><%4>", _damaged, _damage, getDamage _damaged, _reduce_damages]] call CTI_CO_FNC_Log;
	};
};
